...
def year_avg_houseprice(data1,year,dis):
    ...
    # 繪製折線圖
    line_chart = px.line(average_prices, x='月份', y='square_price(萬/坪)',  color='type', line_group='type',title=f'{year}年{dis}單坪房價趨勢圖')
                    ...
                    buttons=list([
                        dict(
                            ...
                            args=[{"visible": [True, True, True]},
                                {"title": f"{year}年{dis}房價趨勢圖 - 全部"}]
                        ),
                        dict(
                            label="華廈",
                            method="update",
                            args=[{"visible": [False, False, True]},
                                {"title": f"{year}年{dis}房價趨勢圖 - 華廈"}]
                        ),
                        dict(
                            label="公寓",
                            method="update",
                            args=[{"visible": [False, True, False]},
                                {"title": f"{year}年{dis}房價趨勢圖 - 公寓"}]
                        ),
                        dict(
                            label="住宅大樓",
                            method="update",
                            args=[{"visible": [True, False, False]},
                                {"title": f"{year}年{dis}房價趨勢圖 - 住宅大樓"}]
                        ),
                    ]),
                    direction="down",
                    showactive=True,
