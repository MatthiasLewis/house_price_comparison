...
# 找到區域點擊下拉選單=>文山區=>立即搜尋
element =driver.find_element(By.XPATH,("//div[@class='selectform selectpush where']/span[text()='全區']"))
element.click()
time.sleep(randint(3,5))
element2 = driver.find_element(By.CSS_SELECTOR, "div.selectform.selectpush.where.active ul.selectpanel.selblock02 li input[value='116']")
element2.click()
time.sleep(randint(3,5))
...
while True:
    for value in result:
        if "收納" in value.text or "店面" in value.text or "店辦" in value.text or "賣場" in value.text:
            continue
    ...
    try:
        ...
        break    
print(data)
with open (f"rent_文山區.json","w",encoding="utf8") as f:
